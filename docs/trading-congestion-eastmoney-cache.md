...
- 当日交易拥挤度的成交额、Top100、换手率、量比默认使用东方财富实时横截面。
- 如当日快照永久缺失，可用 iFinD 全A历史横截面重建成交额集中度及成交额/成交量 Top100；必须按证券代码分片取全、检查无重复，并用独立 Top100 查询逐只匹配。
- iFinD 重建行使用 `ifind_historical_reconstruction` 来源标记；无法可靠获取的换手率、量比和市值字段保持为空，不用当前日数值代填。
