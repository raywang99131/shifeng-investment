...
export async function fetchTreasuryCurve(fetchImpl: typeof fetch, clearingDate: string, now: Date): Promise<TreasuryCurve> {
  ...
  const candidates = lines.slice(1).flatMap((line) => {
    ...
    const nodes: TreasuryCurveNode[] = CURVE_COLUMNS.map(([column, years]) => ({ years, zeroRate: Number(values[headerIndex.get(column)!]) / 100 }));
  ...
  return { curveId: `ust-par-zero-proxy-${selected.asOf}`, asOf: selected.asOf, currency: 'USD', sourceLabel: 'U.S. Treasury par yields · continuous-zero proxy', sourceUrl: TREASURY_CURVE_SOURCE_URL, retrievedAt: now.toISOString(), payloadHash, nodes: selected.nodes };
}
