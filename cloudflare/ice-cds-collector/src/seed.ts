...
async function assertNoImmutableConflicts(db: D1Database, seed: SeedPackage): Promise<void> {
  ...
  for (const batch of seed.publishedBatches) {
    const storedByRevision = await db.prepare(`SELECT batch_id, clearing_date, revision, published_at, source_kind, quality_status FROM published_batches WHERE clearing_date = ? AND revision = ?`).bind(batch.clearingDate, batch.revision).first<Record<string, unknown>>();
    const storedById = await db.prepare(`SELECT batch_id, clearing_date, revision, published_at, source_kind, quality_status FROM published_batches WHERE batch_id = ?`).bind(batch.batchId).first<Record<string, unknown>>();
    if (storedByRevision && storedById && storedByRevision.batch_id !== storedById.batch_id) invalid();
    const stored = storedByRevision ?? storedById;
    if (!stored) continue;
