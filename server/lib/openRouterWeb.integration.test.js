...
test('bundled weekly history is readable offline before the first rankings refresh', async (t) => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'openrouter-offline-'));
  ...
  const service = createAiDashboardServiceFromEnv({
    ...
    fetchImpl: async () => { calls += 1; return new Response('unavailable', { status: 503 }); },
  });
  const initial = await service.getSnapshot();
  assert.equal(calls, 0);
  assert.equal(initial.openRouter.weeklyHistory?.weeks.length, 52);
  assert.equal(initial.openRouter.weeklyHistory.asOf, '2026-09-05');
  const failed = await service.refresh({ sources: ['openRouter'] });
  assert.equal(calls, 1);
  assert.deepEqual(failed.openRouter.weeklyHistory, initial.openRouter.weeklyHistory);
